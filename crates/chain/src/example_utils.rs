#![allow(unused)]
use crate::BlockId;
use alloc::vec::Vec;
use bitcoin::{consensus, hashes::sha256d, BlockHash, Transaction, Txid};

pub const RAW_TX_1: &str = "";
pub const RAW_TX_2: &str = "";
pub const RAW_TX_3: &str = "";
pub const RAW_TX_4: &str = "";

pub fn tx_from_hex(s: &str) -> Transaction {
    consensus::encode::deserialize_hex(s).expect("must deserialize")
}

/// Hash types that wrap a 32-byte array, constructible from arbitrary data for examples.
pub trait ExampleHash {
    fn from_byte_array_32(bytes: [u8; 32]) -> Self;
}

impl ExampleHash for BlockHash {
    fn from_byte_array_32(bytes: [u8; 32]) -> Self {
        BlockHash::from_byte_array(bytes)
    }
}

impl ExampleHash for Txid {
    fn from_byte_array_32(bytes: [u8; 32]) -> Self {
        Txid::from_byte_array(bytes)
    }
}

pub fn new_hash<H: ExampleHash>(s: &str) -> H {
    H::from_byte_array_32(sha256d::Hash::hash(s.as_bytes()).to_byte_array())
}

pub fn new_block_id(height: u32, hash: &str) -> BlockId {
    BlockId {
        height,
        hash: new_hash(hash),
    }
}
